...
humann2_path_noblanks_mr1.pca.variates <- as.data.frame(humann2_path_noblanks_mr1.pca$variates$X)
humann2_path_noblanks_mr1.pca.variates <- humann2_path_noblanks_mr1.pca.variates %>%
  rownames_to_column("SampleID")

humann2_path_noblanks_mr1.pca.varmet <- inner_join(humann2_path_noblanks_mr1.pca.variates, metadata, by = "SampleID")

oral_non_pq_pc12_s <- humann2_path_noblanks_mr1.pca.varmet %>%
  ...
  ggplot(., aes(PC1, PC2, colour = Market_integration_split, shape = Market_integration_split, size = Market_integration_split)) +
    ...
    xlab(paste("PC1 - ", 100*(round(data.frame(humann2_path_noblanks_mr1.pca$prop_expl_var$X[1]), digits = 4)), "%", sep = "")) +
    ylab(paste("PC2 - ", 100*(round(data.frame(humann2_path_noblanks_mr1.pca$prop_expl_var$X[2]), digits = 4)), "%", sep = "")) +
    theme_minimal(base_size = 14) +
    labs(color = "Source", shape = "Source") +
    theme(legend.title = element_text(size = 10))
oral_non_pq_pc12_s

ggsave("../../06-publication/supplemental_figures/SX17/panel_parts/supsub_pca_paths.pdf", plot = oral_non_pq_pc12_s, device = "pdf",
