...
ggsave("./05-results.backup/presentation_pdfs/scree_seed_noblanks_Varimax_minres.pdf", plot = scree_plot, device = "pdf",
...
seed_L4.decontam_FA_all_clr_order <- seed_L4.decontam_FA_all_clr %>%
  ...
  spread(SampleID,CLR) %>%
  mutate(SpOrder = as.character(seq(1,11070))) %>% # MR1 - 1070; MR2 - 637
...
ggsave("./05-results.backup/presentation_pdfs/scree_seed_cmc_Varimax_minres.pdf", plot = scree_plot, device = "pdf",
       scale = 1, width = 10, height = 10, units = c("in"), dpi = 300)
...
seed_L4.decontam_FA_cmc_clr_order <- seed_L4.decontam_FA_cmc_clr %>%
  ...
  gather("Pathway","CLR",2:ncol(.)) %>%
  spread(SampleID,CLR) %>%
  mutate(SpOrder = as.character(seq(1,938))) %>% # MR1 - 938; MR2 - 686
  select(Pathway, SpOrder, everything()) %>%
