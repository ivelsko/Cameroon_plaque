...
seed_L4.decontam_noblanks.fa$values %>%
  as.data.frame(.) %>%
  rename(Eigenvalues = ".") %>%
...
```{r lda_species_decontam_noblanks_me, eval = F}
...
```{r lda_species_decontam_noblanks_sex, eval = F}
...
```{r lda_species_decontam_noblanks_vi, eval = F}
...
```{r heatmap_noblanks_noyanomami, eval = F}
# select the SEED proteins in MR1 and MR2 for the FA
seed_L4.decontam_FA_all <- malt_Pathway.decontam %>%
                                  select(-one_of(outliers)) %>%
                                  select(-matches("SRR164|10M|EXB|LIB")) %>%
                                  inner_join(., minres_oblimin_nobl_noyano_fa_Pathway) %>%
                                  gather("SampleID","Counts",2:ncol(.)) %>%
                                  spread(Pathway,Counts)
...
```{r discriminant_taxa_pca, eval = F}
...
tune.pca(seed_L4.decontam_FA, logratio = 'CLR')

# perform a PCA to see how the data cluster
seed_L4.decontam_FA.pca <- pca(seed_L4.decontam_FA, ncomp = 3, logratio = 'CLR')
...
# from the graphs, I selected fm = minres and rotate = oblimin
seed_L4.decontam_cmc.fa <- fa(seed_L4.decontam_cmc_clr, nfactors = 6, rotate = "Varimax", fm = "minres") # rotate = "Varimax" "oblimin" # fm = "ols" "minres"
# if using fm = "ols", the column titles will be 1, 2, etc and not M1, M2, etc as you get from using fm = "minres"
...
seed_L4.decontam_cmc.fa$values %>%
  as.data.frame(.) %>%
  rename(Eigenvalues = ".") %>%
  arrange(desc(Eigenvalues))%>%
  rownames_to_column("xvalue") %>%
