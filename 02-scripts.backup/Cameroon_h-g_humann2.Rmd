...
plot_pca <- function(df, pc1, pc2, metadata_group) {
    ...
    pca_plot <- ggplot(df_X, aes(pc1, pc2, colour = metadata_group, shape = metadata_group)) +
                        ...
                        # stat_ellipse() +
                        xlab(paste(colnames(pc1), " - ", exp_var[1])) +
                        ylab(paste(colnames(pc2), " - ", exp_var[2])) +
...
plot_pca_bi <- function(df, pc1, pc2, metadata_group, arrow_pc) {
    metadata_group_colors <- get(paste(metadata_group, "_colors", sep = ""))
    metadata_group_shapes <- get(paste(metadata_group, "_shapes", sep = ""))
    ...
    # if (arrow_pc == 'PC1') {
    #     arrow_pc <- df_X$PC1
    # } else if (arrow_pc == 'PC2') {
    #     arrow_pc <- df_X$PC2
    # } else if (arrow_pc == 'PC3') {
    #     arrow_pc <- df_X$PC3
    # }
    ...
    pca_plot_bi <- ggplot(df_X, aes(x = pc1, y = pc2, colour = metadata_group, shape = metadata_group)) +
      geom_point(size = 2) +
      ...
      geom_segment(data = neg_10,
                   ...
                   size = 0.5,
                   arrow = arrow(length = unit(0.03, "npc"))) +
      labs(x = paste(colnames(pc1), " - ", exp_var[1]),
           y = paste(colnames(pc2), " - ", exp_var[2])) +
      scale_colour_manual(values = metadata_group_colors) +
      scale_shape_manual(values = metadata_group_shapes) +
...
pca_test_biplot <- plot_pca_bi(humann2_path.pca, "PC1", "PC2", "Ethnic_Group", "PC1")
pca_test_biplot
...
corr_lam <- humann2_path.pca$sdev[c("PC1", "PC2")] * sqrt(nrow(df_X))

df_X <- df_X %>%
      mutate(PC1 = PC1 / corr_lam[1],
             PC2 = PC2 / corr_lam[2])

pws_10 <- humann2_path.pca$loadings$X %>%
      as.data.frame(.) %>%
      rownames_to_column(var = "seed_id") %>%
      top_n(10, PC1) 
...
Plot the total number of assigned reads in each subsistence strategy, and the
...
# pca_test_plot_names <- plot_pca_names(humann2_path.pca, "PC1", "PC3", "Ethnic_Group")
# pca_test_plot_names
```
