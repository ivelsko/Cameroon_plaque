...
humann2_path <- fread("./05-results.backup/XXXXXXXXXXXXXXXXXXXXXX")
humann2_KOs <- fread("./05-results.backup/XXXXXXXXXXXXXXXXXXXXXX")

# clean the file names
malt_species <- rename(malt_species, Species = `#Datasets`)
colnames(malt_species) <- gsub(".SG1.1_S0_L001_R1_001.fastq.combined.fq.prefixed.extractunmapped.bam","", colnames(humann2_path))
colnames(malt_species) <- gsub("_S0_L001_R1_000.fastq.combined.fq.prefixed.extractunmapped.bam","", colnames(humann2_path))
colnames(malt_species) <- gsub("_S0_L001_R1_001.fastq.combined.fq.prefixed.extractunmapped.bam.10M","_10M", colnames(humann2_path))
colnames(malt_species) <- gsub("_S0_L001_R1_000.10M.fastq.combined.fq.prefixed.extractunmapped.bam","_10M", colnames(humann2_path))
colnames(malt_species) <- gsub("_S0_L001_R1_001.fastq.combined.fq.prefixed.extractunmapped.bam","", colnames(humann2_path))


# remove human and unassigned reads
humann2_path <- humann2_path %>% filter(!str_detect(Species, "Homo sapiens"))
humann2_path <- humann2_path %>% filter(!str_detect(Species, "Not assigned"))
# get rid of the extra column at the begining
humann2_path <- humann2_path %>% select(-CMC_HMP_comparison)
...
Plot the total number of assigned reads in each subsistence strategy, and the proportion of assigned reads in each subsistence strategy. 
```{r}
