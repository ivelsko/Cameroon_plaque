...
metadata <- metadata %>%
  ...
  mutate(Description = fct_relevel(Description, "HunterGatherer", "HMP_supPlaque", "HMP_subPlaque",
         ...
         Sex = fct_relevel(Sex, "Male","Female","HMP","HMP10M","NA"),
         Tooth_site = fct_relevel(Tooth_site, "Anterior","Posterior","HMP","HMP10M"),
         Type = fct_relevel(Type, "Calculus","Plaque","ExtractionGauze","ExtractionBlank","LibraryBlank"),
...
plot_pca <- function(df) {
    ...
    df_X <- df$variates$X %>%
              ...
              inner_join(metadata_sample, by = "SampleID") %>%
              mutate(environment = fct_relevel(environment, "palaeofaeces", "traditional SS", "industrial FP",
                                                            "soil","ancient calculus", "modern calculus"))
    
    pca_plot <- ggplot(df_X, aes(PC1, PC2, colour = environment)) +
                        geom_point(size = 4) +
                        scale_colour_manual(values = c(unique(groups_colours$groupColor), "grey20", "deepskyblue3", "lightskyblue1")) +
                        stat_ellipse() +
