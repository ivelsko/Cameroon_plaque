...
# plotting box and whisker for input
...
# plotting box and whisker for comparing nt vs RefSeq
plot_box_whisker_db <- function(df, stat_info, metadata_group, fill_group, title_text) {
    fill_group_colors <- get(paste(fill_group, "_colors", sep = ""))

    if (df == malt_stats_new) { # this one will plot for test_plot_2 with the error: the condition has length > 1 and only the first element will be used; not for test_plot_3 with the error: Error in Ops.data.frame(df, malt_stats_new) : ‘==’ only defined for equally-sized data frames
    # if (df == "malt_stats_new") { # this one will plot for test_plot_3 with the error: the condition has length > 1 and only the first element will be used; not for test_plot_2 with the error: Error: Aesthetics must be either length 1 or the same as the data (248): x, fil
     df_X <- df
      fill_group = df_X[[fill_group]]
    } else {
      df_X <- df %>%
...
test_plot_2 <- plot_box_whisker_db(malt_stats_new, "Percent", "Env", "Database", "Percent of reads assigned by MALT")
test_plot_2

test_plot_3 <- plot_box_whisker_db(maltnt_assignments, "Percent", "Env", "Env", "Percent of reads assigned by MALT with nt2017")
test_plot_3
...
plot_box_whisker(maltnt_assignments, "Percent", "Env", "Env", "Percent of reads assigned by MALT with nt2017")
plot_box_whisker(maltnt_assignments, "Percent", "Village", "Village", "Percent of reads assigned by MALT with nt2017")
plot_box_whisker(maltnt_assignments, "Queries", "Env", "Env", "Number of reads assigned by MALT with nt2017")
plot_box_whisker(maltnt_assignments, "Queries", "Village", "Village", "Number of reads assigned by MALT with nt2017")

```

```{r graphs_RSC}

plot_box_whisker(malt_CMC_HMP_JAE_RSC_assignments, "Queries", "Env", "Env", "Number of reads submitted to MALT for classification")
plot_box_whisker(malt_CMC_HMP_JAE_RSC_assignments, "Queries", "Village", "Village", "Number of reads submitted to MALT for classification")
plot_box_whisker(malt_CMC_HMP_JAE_RSC_assignments, "Aligned", "Env", "Env", "Number of reads assigned by MALT with RefSeqCustom")
plot_box_whisker(malt_CMC_HMP_JAE_RSC_assignments, "Aligned", "Village", "Village", "Number of reads assigned by MALT with RefSeqCustom")
plot_box_whisker(malt_CMC_HMP_JAE_RSC_assignments, "Percent", "Env", "Env", "Percent of reads assigned by MALT with RefSeqCustom")
plot_box_whisker(malt_CMC_HMP_JAE_RSC_assignments, "Percent", "Village", "Village", "Percent of reads assigned by MALT with RefSeqCustom")
...
db_plot_env <- plot_box_whisker_db(malt_stats_new, "Percent", "Env", "Database", "Percent of reads assigned by MALT with nt2017 and RefSeqCustom")
db_plot_env

db_plot_village <- plot_box_whisker_db(malt_stats_new, "Percent", "Village", "Database", "Percent of reads assigned by MALT with nt2017 and RefSeqCustom")
db_plot_village

# make box plots of the % of assigned reads and of the total # of reads
malt_assignments_plots_pct_group <- malt_stats_new %>%
...
malt_assignments_plots_pct_village <- malt_stats_new %>%
