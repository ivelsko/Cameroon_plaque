...
metadata <- metadata %>%
  ...
  as_tibble() %>%
  mutate(Env = fct_relevel(Env, "HunterGatherer","HMP","HMP10M","JAE","JAE10M",
...
filter_result <- simple_filter(curves, threshold = 81) # threshold = 82 starts to color some of the CMC samples red
...
burnin_result <- hard_burnin_filter(curves, threshold = 81, burnin = 0.9) # threshold = 50, burnin = 0.4 allows the highest ExtractionBlank line to turn turquoise; threshold = 81, burnin 0.1-0.9 keeps all CMC turquoise and all ExtractionBlank red
...
burnin_result <- adaptive_burnin_filter(curves, threshold = 59) # threshold = 50 allows the highest ExtractionBlank line to turn turquoise; threshold = 60 keeps the highest ExtractionBlank line red but turns one CMC sample line red; threshold = 59 keeps all CMC turquoise and all ExtractionBlanks red; going higher turns more CMC red
plot_cuperdec(curves, metadata_map, burnin_result, restrict_x = 250)
