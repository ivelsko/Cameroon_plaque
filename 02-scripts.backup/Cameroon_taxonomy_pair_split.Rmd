...
malt_species.decontam_filtered_noblanks <- malt_species.decontam %>%
                                         ...
                                         select(-one_of(anterior)) %>% # change this between anterior and posterior, depending on which samples you want to remove
...
rownames(malt_species.decontam_filtered_noyanomami) <- malt_species.decontam_filtered_noyanomami$SampleID

malt_species.decontam_filtered_noyanomami <- malt_species.decontam_filtered_noyanomami %>% select(-SampleID)

malt_species.decontam_filtered_noyanomami = malt_species.decontam_filtered_noyanomami+1
...
# by Individual sequencing depth
# perform a PCA to see how the data cluster
malt_species_decontam_filtered_noblanks.pca <- mixOmics::pca(malt_species.decontam_filtered_noyanomami, ncomp = 3, logratio = 'CLR')
plot(malt_species_decontam_filtered_noblanks.pca)
...
adonis2(malt_species.decontam_filtered_cmc.euc ~ ExtractionBatch + Village + Sex + Industry + Ethnic_Group + Age_group +  LibraryBatch + Market_economy, metadata_cmc, permutations = 999, by = "margin")
...
adonis2(malt_genus.decontam_filtered_cmc.euc ~ ExtractionBatch + Village + Sex + Industry + Ethnic_Group + Age_group +  LibraryBatch + Market_economy, metadata_cmc, permutations = 999, by = "margin")
