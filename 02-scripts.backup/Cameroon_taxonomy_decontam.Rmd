...
plot_pca <- function(df, pc1, pc2, metadata_group) {
    ...
    df_X <- df$variates$X %>%
              ...
              inner_join(metadata %>%
                           select(SampleID, Env, Description, Ethnic_Group, Age_group, Sex, Market_integration, Village, Tooth_site, Type), by = "SampleID")
...
malt_species_mat <- malt_species_raw %>%
  pivot_longer(!Species, names_to = "SampleID", values_to = "Counts") %>%
  mutate(Counts = replace_na(Counts, 0), 
         Counts = Counts + 1) %>%
  pivot_wider(names_from = "Species", values_from = "Counts", values_fill = 1) %>% 
  column_to_rownames("SampleID")

# prepare to run a PCA
# check the number of components to retain by tuning the PCA
tune.malt_species_pca <- mixOmics::tune.pca(malt_species_mat, logratio = 'CLR')
tune.malt_species_pca

# perform a PCA to see how the data cluster
malt_species.pca <- mixOmics::pca(malt_species_mat, ncomp = 3, logratio = 'CLR')
...
malt_species.dataframe <- malt_species_raw %>%
  filter(!str_detect(Species, "Not assigned")) %>%
  select(-one_of(outliers)) %>%
  select(matches("Species|CMC|EXB|LIB")) %>%
  adorn_totals(where = "col") %>%
  filter(Total > 0) %>%
  select(-Total) %>%
...
cowplot::plot_grid(sp_curve_HG, sp_curve_EG,
...
# 
# cowplot::plot_grid(gn_curve_HG, gn_curve_EG,
...
```{r}
malt_species.matrix %>% as.data.frame(.) %>% rownames_to_column("SampleID") %>% select(SampleID) %>% anti_join(., metadata.decontam %>% select(SampleID))

metadata.decontam %>% select(SampleID) %>% anti_join(., malt_species.matrix %>% as.data.frame(.) %>% rownames_to_column("SampleID") %>% select(SampleID))


malt_species.matrix %>% as.data.frame(.) %>% rownames_to_column("SampleID") %>% select(SampleID) %>% bind_cols(., metadata.decontam %>% select(SampleID) %>% rename(SampleID_metadata = SampleID)) %>% mutate(sameID = SampleID == SampleID_metadata)
...
metadata.decontam <- metadata.decontam %>%
  select(SampleID, Sample_or_Control, SumLibraryConcentration_copies_ul, is.control)
...
cowplot::plot_grid(sp_decontam_curve_HG, sp_decontam_curve_EG, sp_decontam_curve_EB, sp_decontam_curve_LB, sp_decontam_curve_Yano, sp_decontam_curve_HMPsup,
...
# 
# cowplot::plot_grid(gn_decontam_curve_HG, gn_decontam_curve_EG, gn_decontam_curve_EB, gn_decontam_curve_LB, gn_decontam_curve_Yano,
...
decontaminated input table for analysis. *Note that the genus list includes*
*Actinomyces, and this needs to be removed from the list b/c Actinomyces are a*
*well known oral genus*
