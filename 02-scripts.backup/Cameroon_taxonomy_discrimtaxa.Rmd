...
malt_species.decontam_filtered_noblanks.fa <- fa(malt_species.decontam_filtered_noblanks_clr, nfactors = 5, rotate = "varimax", fm = "minres") # fm = "ols"
...
```{r lda_species_decontam_noblanks}
...
malt_species.decontam_filtered_noblanks_lda_table <- malt_species.decontam_filtered_noblanks_clr %>%
  ...
  inner_join(., metadata_noblanks %>%
               ...
               # Age_group, Ethnic_Group, Env, Market_economy, Sex, Tooth_site, Village
...
malt_species.decontam_filtered_noblanks_lda <- lda(Ethnic_Group ~ ., malt_species.decontam_filtered_noblanks_lda_table)
...
ggplot(lda.data, aes(LD1, LD2, colour = Ethnic_Group, shape = Ethnic_Group)) +
                        geom_point(size = 4) +
                        scale_colour_manual(values = Ethnic_Group_colors) +
                        scale_shape_manual(values = Ethnic_Group_shapes) +
                        xlab("LD1") +
...
malt_species.decontam_filtered_noblanks_noyanomami.fa <- fa(malt_species.decontam_filtered_noblanks_noyanomami_clr, nfactors = 5, rotate = "varimax", fm = "minres") # fm = "ols"
...
malt_species.decontam_filtered_noblanks_noyanomami.fa.df <- malt_species.decontam_filtered_noblanks_noyanomami.fa.df %>%
  ...
  gather("Group","Residual", 7:ncol(.)) %>%
  filter(Residual != "0") %>%
  select(Residual, Species, MR1, MR2, MR4, MR3, MR5)

head(malt_species.decontam_filtered_noblanks_noyanomami.fa.df)

```

```{r da_species_decontam_noblanks_noyanomami}
...
malt_species.decontam_filtered_cmc.fa <- fa(malt_species.decontam_filtered_cmc_clr, nfactors = 5, rotate = "varimax", fm = "minres") # fm = "ols"
...
# this won't work with only 2 groups. Need to make another graph. Follow bar charts of plot()?
# ggplot(lda.data, aes(LD1, LD2, colour = Ethnic_Group, shape = Ethnic_Group)) +
#                         geom_point(size = 4) +
#                         scale_colour_manual(values = Ethnic_Group_colors) +
#                         scale_shape_manual(values = Ethnic_Group_shapes) +
