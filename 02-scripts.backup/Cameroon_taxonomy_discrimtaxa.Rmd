...
# CMC + Industrial + Yanomami
```{r fa_species_decontam_noblanks, eval = F}
...
```{r lda_species_decontam_noblanks_eg, eval = F}
...
```{r fa_species_decontam_noblanks_noyanomami, eval = F}
# from the graphs, I selected fm = minres and rotate = oblimin
malt_species.decontam_filtered_noblanks_noyanomami.fa <- fa(malt_species.decontam_filtered_noblanks_noyanomami_clr, nfactors = 7, rotate = "oblimin", fm = "minres") # rotate = "varimax" "Varimax" "oblimin" # fm = "ols" "minres"
...
colnames(graphinfo) <- c("MR1","MR2","MR3","MR4","MR5","MR6","MR7")
graphinfo <- graphinfo %>%
  rownames_to_column("Species") %>%
  as_tibble(.)
...
fwrite(malt_species.decontam_filtered_noblanks_noyanomami.fa_species, file = "./05-results.backup/minres_oblimin_noyanomami.fa_species.tsv", quote = F, sep = "\t")

```

```{r lda_species_decontam_noblanks_noyanomami_eg, eval = F}
...
```{r lda_species_decontam_noblanks_noyanomami_age, eval = F}
# select only the species that make up the 1st 2 residuals, based on the scree plot above
fa_species_noblanks_noyanomami <- malt_species.decontam_filtered_noblanks_noyanomami.fa.df %>% 
  filter(Residual %in% c("MR1","MR2")) %>%
  pull(Species)
...
```{r lda_species_decontam_noblanks_noyanomami_me, eval = F}
# select only the species that make up the 1st 2 residuals, based on the scree plot above
fa_species_noblanks_noyanomami <- malt_species.decontam_filtered_noblanks_noyanomami.fa.df %>% 
  filter(Residual %in% c("MR1","MR2")) %>%
  pull(Species)
...
```{r lda_species_decontam_noblanks_noyanomami_sex, eval = F}
# select only the species that make up the 1st 2 residuals, based on the scree plot above
fa_species_noblanks_noyanomami <- malt_species.decontam_filtered_noblanks_noyanomami.fa.df %>% 
  filter(Residual %in% c("MR1","MR2")) %>%
  pull(Species)
...
```{r lda_species_decontam_noblanks_noyanomami_ts, eval = F}
# select only the species that make up the 1st 2 residuals, based on the scree plot above
fa_species_noblanks_noyanomami <- malt_species.decontam_filtered_noblanks_noyanomami.fa.df %>% 
  filter(Residual %in% c("MR1","MR2")) %>%
  pull(Species)
...
```{r lda_species_decontam_noblanks_noyanomami_vi, eval = F}
# select only the species that make up the 1st 2 residuals, based on the scree plot above
fa_species_noblanks_noyanomami <- malt_species.decontam_filtered_noblanks_noyanomami.fa.df %>% 
  filter(Residual %in% c("MR1","MR2")) %>%
  pull(Species)
...
rownames(malt_species.decontam_FA_all) <- malt_species.decontam_FA_all$SampleID
malt_species.decontam_FA_all <- malt_species.decontam_FA_all %>% select(-SampleID)

# CLR-transform the matrix for plotting the heatmap
malt_species.decontam_FA_all_clr = malt_species.decontam_FA_all+1
malt_species.decontam_FA_all_clr <- clr(malt_species.decontam_FA_all_clr)
...
malt_species.decontam_FA_all_clr_order <- malt_species.decontam_FA_all_clr %>%
  ...
  spread(SampleID,CLR) %>%
  mutate(SpOrder = as.character(seq(1,137))) %>%
...
heatmap_base <- ggplot(malt_species.decontam_FA_all_clr_long, aes(SampleID, Species, fill = CLR)) +
  geom_tile() +
...
rownames(malt_species.decontam_filtered_noblanks_FA) <- malt_species.decontam_filtered_noblanks_FA$SampleID

malt_species.decontam_filtered_noblanks_FA <- malt_species.decontam_filtered_noblanks_FA %>% select(-SampleID)

malt_species.decontam_filtered_noblanks_FA = malt_species.decontam_filtered_noblanks_FA+1
...
# by Village
noblanks_fa_pca <- plot_pca(malt_species.decontam_filtered_noblanks_FA.pca, "PC1", "PC2", "Tooth_site")
noblanks_fa_pca
plot_pca(malt_species.decontam_filtered_noblanks_FA.pca, "PC3", "PC2", "Tooth_site")
...
```{r fa_species_decontam_cmc, eval = F}
...
```{r lda_species_decontam_cmc_eg, eval = F}
...
```{r lda_species_decontam_cmc_age, eval = F}
...
```{r lda_species_decontam_cmc_me, eval = F}
...
```{r lda_species_decontam_cmc_sex, eval = F}
...
```{r lda_species_decontam_cmc_ts, eval = F}
...
```{r lda_species_decontam_cmc_village, eval = F}
...
rownames(malt_species.decontam_FA_cmc) <- malt_species.decontam_FA_cmc$SampleID
malt_species.decontam_FA_cmc <- malt_species.decontam_FA_cmc %>% select(-SampleID)

# CLR-transform the matrix for plotting the heatmap
malt_species.decontam_FA_cmc_clr = malt_species.decontam_FA_cmc+1
malt_species.decontam_FA_cmc_clr <- clr(malt_species.decontam_FA_cmc_clr)
...
malt_species.decontam_FA_cmc_clr_order <- malt_species.decontam_FA_cmc_clr %>%
  ...
  gather("Species","CLR",2:ncol(.)) %>%
  spread(SampleID,CLR) %>%
  mutate(SpOrder = as.character(seq(1,198))) %>%
  select(Species, SpOrder, everything()) %>%
...
# make a heatmap with ggplot
heatmap_base <- ggplot(malt_species.decontam_FA_cmc_clr_long, aes(SampleID, Species, fill = CLR)) +
  geom_tile() +
  scale_fill_viridis_c(option = "A") +
  theme_minimal(base_size = 7) +
  theme(axis.text.x = element_text(angle = 90, hjust = 0.95),
...
rownames(malt_species.decontam_filtered_cmc_FA) <- malt_species.decontam_filtered_cmc_FA$SampleID

malt_species.decontam_filtered_cmc_FA <- malt_species.decontam_filtered_cmc_FA %>% select(-SampleID)

malt_species.decontam_filtered_cmc_FA = malt_species.decontam_filtered_cmc_FA+1
