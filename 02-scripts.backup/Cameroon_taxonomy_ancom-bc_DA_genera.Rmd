---
title: "Cameroon calculus/plaque MALT differentially abundant genera by ANCOM-BC"
author: "Irina Velsko"
...
Read in the tables that have the genus that are in MR1, for filtering the results later
```{r mr_genus}
# read in the files that have the MR1, MR2, MR3 genus/genera
mr_genus <- fread("./05-results.backup/minres_oblimin_noyanomami.fa_genus.tsv")
mr_cmc_genus <- fread("./05-results.backup/minres_oblimin_cmc.fa_genus.tsv")
...
# ANCOM-BC
# Industrial and Cameroon plaque samples

## Env
```{r ancomBC_noblanks_env}

otu_data <- malt_genus.decontam_filtered_noblanks_noyanomami
taxonomy = otu_data$Genus
otu_data <- otu_data %>%
...
res_noblanks.ANCOM_BC <- data.frame(genus=rownames(out_noblanks$feature.table), out_noblanks$res, 
...
res_noblanks.ANCOM_BC <- res_noblanks.ANCOM_BC %>% transmute(genus,
...
fwrite(ancom_bc_noblanks_HMP_HG, "./05-results.backup/ancomBC_genus_noblanks_HMP_HG.tsv", sep = "\t", quote = F) # equivalent to ma_us_phylum_age2.csv in the example below

res.ANCOM_BC <- data.frame(genus=rownames(out_noblanks$feature.table), out_noblanks$res, 
                        struc.zero[rownames(out_noblanks$feature.table), ], 
                        row.names = NULL, stringsAsFactors=FALSE, check.names = FALSE)
...
ancom_bc_noblanks_JAE_HG <- res.ANCOM_BC %>%
  ...
  arrange(q.val) %>%
  filter(diff.abn == "TRUE")
  # possiblly filter out the genus that have a differential abundance of less than |1|
  # filter(`log fold change (HunterGatherer - JAE)` < -1 | `log fold change (HunterGatherer - JAE)` > 1)
  
fwrite(ancom_bc_noblanks_JAE_HG, "./05-results.backup/ancomBC_genus_noblanks_JAE_HG.tsv", sep = "\t", quote = F)
...
# filter the table to include only the genus that are in MR1 (if any)
...
# list the genus shared between the 2 comparisons
Env_genus = unique(c(ancom_bc_noblanks_JAE_HG_mr1$Genus, ancom_bc_noblanks_HMP_HG_mr1$Genus))
...
p1 = ggplot(ancom_bc_noblanks_env_mr1, aes(x=Genus, y=log_fold_change, ymin=ci.lo, ymax=ci.up, group=Env)) + 
  ...
  scale_x_discrete(limits = rev(levels(ancom_bc_noblanks_env_mr1$Genus)))+
  theme_bw()+
...
p1 = p1+geom_point(data = ancom_bc_noblanks_env_mr1 %>% filter(struct.zero == 1), aes(x=Genus, y=log_fold_change),
...
otu_data <- malt_genus.decontam_filtered_cmc
taxonomy = otu_data$Genus
otu_data <- otu_data %>%
  column_to_rownames("Genus")
...
# fwrite(res_cmc, "./05-results.backup/ancomBC_cmc_ethnicgroup.tsv", sep = "\t", quote = F)

# Test result (taken from https://github.com/FrederickHuangLin/ANCOM-BC/blob/master/scripts/figure_6_table_s1_s2_s3.Rmd)
res.ANCOM_BC <- data.frame(genus=rownames(out_cmc$feature.table), out_cmc$res, 
...
fwrite(ancom_bc_cmc_Nz_Ba, "./05-results.backup/ancomBC_genus_cmc_eg.tsv", sep = "\t", quote = F)
...
# list the genus shared between the 2 comparisons
Env_genus = unique(ancom_bc_cmc_eg_mr1$Genus)
...
# plot the fold change as a bar plot (also adapted from (https://github.com/FrederickHuangLin/ANCOM-BC/blob/master/scripts/figure_6_table_s1_s2_s3.Rmd))  
p1 = ggplot(ancom_bc_cmc_eg_mr1, aes(x=Genus, y=log_fold_change, ymin=ci.lo, ymax=ci.up, group=Ethnic_Group)) + 
  geom_bar(aes(fill=Ethnic_Group), stat="identity", width=0.4, position=position_dodge())+
  geom_errorbar(width=0.2, size=0.25, position=position_dodge(width = 0.4))+
  geom_abline(slope = 0, intercept = 0, linetype="dashed", alpha=0.5)+
  labs(x=NULL, y="Log Fold Change")+coord_flip()+
  scale_fill_discrete(name=NULL)+
  scale_x_discrete(limits = rev(levels(ancom_bc_cmc_eg_mr1$Genus)))+
  theme_bw()+
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        strip.background = element_rect(fill="white"))+
...
p1 = p1+geom_point(data = ancom_bc_cmc_eg_mr1 %>% filter(struct.zero == 1), aes(x=Genus, y=log_fold_change),
                   position=position_dodge(width = 0.4), shape=18)
...
fwrite(ancom_bc_cmc_Fa_FC, "./05-results.backup/ancomBC_genus_cmc_Fa_FC.tsv", sep = "\t", quote = F)
...
# I think in the structural zero columns, 0 indicates False (not a structural 0) and 1 indicates True (is a structural 0)
ancom_bc_cmc_Fa_LR <- res.ANCOM_BC %>%
  rename(Genus = genus) %>%
  arrange(q.val) %>%
  filter(diff.abn == "TRUE")
  # filter(`log fold change (LoggingRoad - Farming)` < -1 | `log fold change (LoggingRoad - Farming)` > 1)

fwrite(ancom_bc_cmc_Fa_LR, "./05-results.backup/ancomBC_genus_cmc_Fa_LR.tsv", sep = "\t", quote = F)
...
# list the genus shared between the 2 comparisons
Env_genus = unique(c(ancom_bc_cmc_Fa_FC_mr1$Genus, ancom_bc_cmc_Fa_LR_mr1$Genus))
...
# plot the fold change as a bar plot (also adapted from (https://github.com/FrederickHuangLin/ANCOM-BC/blob/master/scripts/figure_6_table_s1_s2_s3.Rmd))  
p1 = ggplot(ancom_bc_cmc_mareco_mr1, aes(x=Genus, y=log_fold_change, ymin=ci.lo, ymax=ci.up, group=Market_economy)) + 
  geom_bar(aes(fill=Market_economy), stat="identity", width=0.4, position=position_dodge())+
  geom_errorbar(width=0.2, size=0.25, position=position_dodge(width = 0.4))+
  geom_abline(slope = 0, intercept = 0, linetype="dashed", alpha=0.5)+
  labs(x=NULL, y="Log Fold Change")+coord_flip()+
  scale_fill_discrete(name=NULL)+
  scale_x_discrete(limits = rev(levels(ancom_bc_cmc_mareco_mr1$Genus)))+
  theme_bw()+
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        strip.background = element_rect(fill="white"))+
...
p1 = p1+geom_point(data = ancom_bc_cmc_mareco_mr1 %>% filter(struct.zero == 1), aes(x=Genus, y=log_fold_change),
                   position=position_dodge(width = 0.4), shape=18)
...
fwrite(ancom_bc_cmc_An_Po, "./05-results.backup/ancomBC_genus_cmc_toothsite.tsv", sep = "\t", quote = F)
...
# list the genus shared between the 2 comparisons
Env_genus = unique(ancom_bc_cmc_ts_mr1$Genus)
...
# plot the fold change as a bar plot (also adapted from (https://github.com/FrederickHuangLin/ANCOM-BC/blob/master/scripts/figure_6_table_s1_s2_s3.Rmd))  
p1 = ggplot(ancom_bc_cmc_ts_mr1, aes(x=Genus, y=log_fold_change, ymin=ci.lo, ymax=ci.up, group=Tooth_site)) + 
  geom_bar(aes(fill=Tooth_site), stat="identity", width=0.4, position=position_dodge())+
  geom_errorbar(width=0.2, size=0.25, position=position_dodge(width = 0.4))+
  geom_abline(slope = 0, intercept = 0, linetype="dashed", alpha=0.5)+
  labs(x=NULL, y="Log Fold Change")+coord_flip()+
  scale_fill_discrete(name=NULL)+
  scale_x_discrete(limits = rev(levels(ancom_bc_cmc_ts_mr1$Genus)))+
  theme_bw()+
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        strip.background = element_rect(fill="white"))+
...
p1 = p1+geom_point(data = ancom_bc_cmc_ts_mr1 %>% filter(struct.zero == 1), aes(x=Genus, y=log_fold_change),
                   position=position_dodge(width = 0.4), shape=18)
...
fwrite(ancom_bc_cmc_M_F, "./05-results.backup/ancomBC_genus_cmc_sex.tsv", sep = "\t", quote = F)
...
# list the genus shared between the 2 comparisons
Env_genus = unique(ancom_bc_cmc_sex_mr1$Genus)
...
# plot the fold change as a bar plot (also adapted from (https://github.com/FrederickHuangLin/ANCOM-BC/blob/master/scripts/figure_6_table_s1_s2_s3.Rmd))  
p1 = ggplot(ancom_bc_cmc_sex_mr1, aes(x=Genus, y=log_fold_change, ymin=ci.lo, ymax=ci.up, group=Sex)) + 
...
p1 = p1+geom_point(data = ancom_bc_cmc_sex_mr1 %>% filter(struct.zero == 1), aes(x=Genus, y=log_fold_change),
                   position=position_dodge(width = 0.4), shape=18)
