...
ggsave("./06-publication/supplemental_figures/SXX26/panel_parts/SXX26_humann2_path_decontam.pdf", plot = humann2_path.decontam_noblanks_long_box, device = "pdf",
       scale = 1, width = 6, height = 7, units = c("in"), dpi = 300)
...
pca_V_plot <- plot_pca(humann2_path_noyanomami.pca, "PC1", "PC2", "Village", "Ethnic_Group", "Ethnic_Group")
pca_V_plot
...
# test for difference between different metadata categories 
adonis2(humann2_path_noyanomami_mat.euc ~ ExtractionBatch +  LibraryBatch, humann2_path_noyanomami_mat.euc.vecmet, permutations = 999, by = "margin")
adonis2(humann2_path_noyanomami_mat.euc ~ Village + Sex + Industry + Tooth_site + Ethnic_Group + Age_group + Market_economy, humann2_path_noyanomami_mat.euc.vecmet, permutations = 999, by = "margin")

# controlled for tooth site
adonis2(humann2_path_noyanomami_mat.euc ~ Village + Sex + Industry + Ethnic_Group + Age_group + Market_economy, humann2_path_noyanomami_mat.euc.vecmet, permutations = 999, by = "margin", strata = metadata_noblanks$Tooth_site)

```
Tooth site (p = 0.001) and village (p < 0.01) are significantly different. When
controlling for tooth site, village is significant (p < 0.01).
...
pca_TS_plot <- plot_pca(humann2_path_cmc.pca, "PC1", "PC2", "Tooth_site", "Ethnic_Group", "Ethnic_Group")
pca_TS_plot
...
pca_V_plot <- plot_pca(humann2_path_cmc.pca, "PC1", "PC2", "Village", "Ethnic_Group", "Ethnic_Group")
pca_V_plot
...
# test for difference between different metadata categories 
adonis2(humann2_path_cmc_mat.euc ~ ExtractionBatch +  LibraryBatch, humann2_path_cmc_mat.euc.vecmet, permutations = 999, by = "margin")
adonis2(humann2_path_cmc_mat.euc ~ Village + Sex + Tooth_site + Ethnic_Group + Age_group +  Market_economy, humann2_path_cmc_mat.euc.vecmet, permutations = 999, by = "margin")

# controlled for tooth site
adonis2(humann2_path_cmc_mat.euc ~ Village + Sex + Ethnic_Group + Age_group + Market_economy, humann2_path_cmc_mat.euc.vecmet, permutations = 999, by = "margin", strata = humann2_path_cmc_mat.euc$Tooth_site)

```
Tooth site and village (both p < 0.01) are significantly different, but when
controlling for tooth site only village is significantly different.
...
PWY-7254: TCA cycle VII (acetate-producers) - used by acetic acid bacteria are Gram-negative acidophilic α-proteobacteria that oxidize ethanol to acetate
...
humann2_KO_box <- humann2_KO_long %>%
  filter(!str_detect(SampleID, "EXB|LIB|SRR164|10M")) %>%
  ggplot(., aes(Village, Orthologs, fill = Village)) +
    geom_boxplot() +
    scale_fill_manual(values = Village_colors) +
    theme_minimal() +
    theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
...
ggsave("./06-publication/supplemental_figures/SXX27/panel_parts/SXX27_humann2_ko_total.pdf", plot = humann2_KO_box, device = "pdf",
       scale = 1, width = 6, height = 7, units = c("in"), dpi = 300)

humann2_KO_long_w <- humann2_KO_long %>%
  filter(!str_detect(SampleID, "EXB|LIB|SRR164|10M")) 
pairwise.wilcox.test(humann2_KO_long_w$Orthologs, humann2_KO_long_w$Village, p.adjust.method = "fdr")
...
humann2_KOs.decontam_long_w <- humann2_KOs.decontam_long %>%
  filter(!str_detect(SampleID, "EXB|LIB|10M")) 
pairwise.wilcox.test(humann2_KOs.decontam_long_w$Orthologs, humann2_KOs.decontam_long_w$Village, p.adjust.method = "fdr")
...
humann2_KOs.decontam_box <- humann2_KOs.decontam_long %>%
  ...
  ggplot(., aes(Village, Orthologs, fill = Village)) +
    ...
    theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
    ylab("Number of KEGG orthologs detected") +
    ggtitle("Number of KEGG orthologs ID'd by HUMAnN2 in MR1")
humann2_KOs.decontam_box
...
This is true for the full table, the decontaminated table. and the MR1 orthologs
table.
...
# create the input matrix
humann2_KO_noblanks_mat2 <- humann2_KOs.decontam %>%
  select(-matches("10M")) %>%
  gather("SampleID","CPM", 2:ncol(.)) %>%
  mutate(CPM = CPM + 1) %>%
  spread(Ortholog, CPM) %>%
...
# check the number of components to retain by tuning the PCA
tune.pca(humann2_KO_noblanks_mat2, logratio = 'CLR')

# perform a PCA to see how the data cluster
humann2_KO_noblanks2.pca <- pca(humann2_KO_noblanks_mat2, ncomp = 3, logratio = 'CLR')

# plot the PCA
pca_EG_plot <- plot_pca(humann2_KO_noblanks2.pca, "PC1", "PC2", "Ethnic_Group", "Ethnic_Group", "Ethnic_Group")
pca_EG_plot
...
How does the relationship of the samples change when we filter out orthologs that
...
h2_mr1_village <- plot_pca(humann2_KOs.decontam_mr1.pca, "PC1", "PC2", "Village", "Ethnic_Group", "Ethnic_Group")
h2_mr1_village

ggsave("./06-publication/supplemental_figures/SXX27/panel_parts/SXX27_humann2_ko_pc12.pdf", plot = h2_mr1_village, device = "pdf",
       scale = 1, width = 6, height = 7, units = c("in"), dpi = 300)
...
# test for difference between different metadata categories 
adonis2(humann2_KO_mat.euc ~ ExtractionBatch + LibraryBatch, humann2_KO_mat.euc.vecmet, permutations = 999, by = "margin")
adonis2(humann2_KO_mat.euc ~ Village + Sex + Industry + Tooth_site + Ethnic_Group + Age_group +  Market_economy, humann2_KO_mat.euc.vecmet, permutations = 999, by = "margin")

# controlled for tooth site
adonis2(humann2_KO_mat.euc ~ Village + Sex + Industry + Ethnic_Group + Age_group + Market_economy, humann2_KO_mat.euc.vecmet, permutations = 999, by = "margin", strata = humann2_KO_mat.euc.vecmet$Tooth_site)

```
When using the MR1 orthologs, there are significant differences between tooth
site, but none when controlling for tooth site.  If using the abundance cut-off,
only tooth site is significantly different (p < 0.01), and when controlling for
tooth site, none are significantly different.
...
h2_mr1_ts <- plot_pca(humann2_KO_cmc.pca, "PC1", "PC2", "Tooth_site", "Tooth_site", "Tooth_site")
h2_mr1_ts
h2_mr1_village <- plot_pca(humann2_KO_cmc.pca, "PC1", "PC2", "Village", "Village", "Village")
h2_mr1_village
...
h2_mr1_ts <- plot_pca(humann2_KOs.decontam_cmc_mr1.pca, "PC1", "PC2", "Tooth_site", "Tooth_site", "Tooth_site")
h2_mr1_ts
h2_mr1_village <- plot_pca(humann2_KOs.decontam_cmc_mr1.pca, "PC1", "PC2", "Village", "Village", "Village")
h2_mr1_village

ggsave("./06-publication/supplemental_figures/SXX27/panel_parts/SXX27_humann2_ko_cmc_pc12.pdf", plot = h2_mr1_village, device = "pdf",
       scale = 1, width = 6, height = 7, units = c("in"), dpi = 300)
...
adonis2(humann2_KO_cmc_mat.euc ~ Village + Sex + Tooth_site + Ethnic_Group + Age_group + Market_economy, humann2_KO_cmc_mat.euc.vecmet, permutations = 999, by = "margin")

# controlled for tooth site
adonis2(humann2_KO_cmc_mat.euc ~ Village + Sex + Ethnic_Group + Age_group + Market_economy, humann2_KO_cmc_mat.euc.vecmet, permutations = 999, by = "margin", strata = humann2_KO_cmc_mat.euc.vecmet$Tooth_site)

```
When using only the orthologs in MR1 from the factor analysis, only tooth site
is significantly different, and none when controlling for tooth site. When using
the abundance cut-off, only tooth site is significantly different (p < 0.01),
and when controlling for tooth site, only sex is significantly different (p <
0.05).
...
K03578 - ATP-dependent helicase HrpA [EC:3.6.4.13]; DNA replication and repair`8
...
# test for difference between different metadata categories 
adonis2(humann2_KO_carbs_mat.euc ~ ExtractionBatch + LibraryBatch, humann2_KO_carbs_mat.euc.vecmet, permutations = 999, by = "margin")
adonis2(humann2_KO_carbs_mat.euc ~ Village + Sex + Industry + Tooth_site + Ethnic_Group + Age_group + Market_economy, humann2_KO_carbs_mat.euc.vecmet, permutations = 999, by = "margin")

# controlled for tooth site
adonis2(humann2_KO_carbs_mat.euc ~ Village + Sex + Industry + Ethnic_Group + Age_group + Market_economy, humann2_KO_carbs_mat.euc.vecmet, permutations = 999, by = "margin", strata = humann2_KO_carbs_mat.euc.vecmet$Tooth_site)

```
Only village (p < 0.05) and tooth site (p < 0.01) are significantly different.
When controlling for tooth site, no village (p < 0.05) is different.
...
# test for difference between different metadata categories 
adonis2(humann2_KO_cmc_carbs_mat.euc ~ ExtractionBatch + LibraryBatch, humann2_KO_cmc_carbs_mat.euc.vecmet, permutations = 999, by = "margin")
adonis2(humann2_KO_cmc_carbs_mat.euc ~ Village + Sex + Tooth_site + Ethnic_Group + Age_group + Market_economy, humann2_KO_cmc_carbs_mat.euc.vecmet, permutations = 999, by = "margin")

# controlled for tooth site
adonis2(humann2_KO_cmc_carbs_mat.euc ~ Village + Sex + Ethnic_Group + Age_group + Market_economy, humann2_KO_cmc_carbs_mat.euc.vecmet, permutations = 999, by = "margin", strata = humann2_KO_cmc_carbs_mat.euc.vecmet$Tooth_site)

```
Only village (p < 0.05) and tooth site (p < 0.01) are significantly different.
When controlling for tooth site, no categories are different.
...
_*All samples*_
_PC1+:_
K01848 - methylmalonyl-CoA mutase, N-terminal domain [EC:5.4.99.2] E5.4.99.2A, mcmA1; Glyoxylate and dicarboxylate metabolism
K15633 - 2,3-bisphosphoglycerate-independent phosphoglycerate mutase [EC:5.4.2.12] gpmI; Glycolysis / Gluconeogenesis
K00209 - enoyl-[acyl-carrier protein] reductase / trans-2-enoyl-CoA reductase (NAD+) [EC:1.3.1.9 1.3.1.44] fabV, ter; Butanoate metabolism
K01678 - fumarate hydratase subunit beta [EC:4.2.1.2] E4.2.1.2AB, fumB; Citrate cycle (TCA cycle)
K01849 - methylmalonyl-CoA mutase, C-terminal domain [EC:5.4.99.2] E5.4.99.2B, mcmA2; Glyoxylate and dicarboxylate metabolism
K03737 - pyruvate-ferredoxin/flavodoxin oxidoreductase [EC:1.2.7.1 1.2.7.-], por, nifJ; Glycolysis / Gluconeogenesis
K00030 - isocitrate dehydrogenase (NAD+) [EC:1.1.1.41], IDH3; Citrate cycle (TCA cycle)
K01711 - GDPmannose 4,6-dehydratase [EC:4.2.1.47], gmd, GMDS; Fructose and mannose metabolism
K01571 - oxaloacetate decarboxylase (Na+ extruding) subunit alpha [EC:7.2.4.2], oadA; Pyruvate metabolism
K01858 - myo-inositol-1-phosphate synthase [EC:5.5.1.4], INO1, ISYNA1; Inositol phosphate metabolism

_PC1-:_
K02821 - ascorbate PTS system EIIA or EIIAB component [EC:2.7.1.194], ulaC, sgaA; Ascorbate and aldarate metabolism
K02783 - glucitol/sorbitol PTS system EIIC component, srlA; Fructose and mannose metabolism
K01433 - formyltetrahydrofolate deformylase [EC:3.5.1.10], purU; Glyoxylate and dicarboxylate metabolism
K03335 - inosose dehydratase [EC:4.2.1.44], iolE; Inositol phosphate metabolism
K02782 - glucitol/sorbitol PTS system EIIB component [EC:2.7.1.198], srlE; Fructose and mannose metabolism
K00068 - sorbitol-6-phosphate 2-dehydrogenase [EC:1.1.1.140], srlE; Fructose and mannose metabolism
K13788 - phosphate acetyltransferase [EC:2.3.1.8], pta; Pyruvate metabolism
K07248 - lactaldehyde dehydrogenase / glycolaldehyde dehydrogenase [EC:1.2.1.22 1.2.1.21], aldA; Pyruvate metabolism
K00010 - myo-inositol 2-dehydrogenase / D-chiro-inositol 1-dehydrogenase [EC:1.1.1.18 1.1.1.369], iolG; Inositol phosphate metabolism
K09788 - 2-methylaconitate isomerase [EC:5.3.3.-], prpF; Propanoate metabolism

_PC2+:_
K00008 - L-iditol 2-dehydrogenase [EC:1.1.1.14], SORD, gutB; Pentose and glucuronate interconversions
K02438 - glycogen debranching enzyme [EC:3.2.1.196], glgX; Starch and sucrose metabolism
K17489 - methylmalonyl-CoA carboxyltransferase 12S subunit [EC:2.1.3.1], E2.1.3.1-12S; Propanoate metabolism
K13922 - propionaldehyde dehydrogenase [EC:1.2.1.87], pduP; Propanoate metabolism
K16148 - alpha-maltose-1-phosphate synthase [EC:2.4.1.342], glgM; Starch and sucrose metabolism
K06044 - (1->4)-alpha-D-glucan 1-alpha-D-glucosylmutase [EC:5.4.99.15], treY, glgY; Starch and sucrose metabolism
K03338 - 5-dehydro-2-deoxygluconokinase [EC:2.7.1.92], iolC; Inositol phosphate metabolism
K05341 - amylosucrase [EC:2.4.1.4], E2.4.1.4; Starch and sucrose metabolism
K03336 - 3D-(3,5/4)-trihydroxycyclohexane-1,2-dione acylhydrolase (decyclizing) [EC:3.7.1.22], iolD; Inositol phosphate metabolism
K01236 - maltooligosyltrehalose trehalohydrolase [EC:3.2.1.141], treZ, glgZ; Starch and sucrose metabolism

_PC2-:_
K18369 - alcohol dehydrogenase [EC:1.1.1.-], adh2; Propanoate metabolism
K00123 - formate dehydrogenase major subunit [EC:1.17.1.9], fdoG, fdhF, fdwA; Glyoxylate and dicarboxylate metabolism
K01792 - glucose-6-phosphate 1-epimerase [EC:5.1.3.15], E5.1.3.15; Glycolysis / Gluconeogenesis
K01654 - N-acetylneuraminate synthase [EC:2.5.1.56], neuB, nnaB; Amino sugar and nucleotide sugar metabolism
K11258 - acetolactate synthase II small subunit [EC:2.2.1.6], ilvM; Butanoate metabolism
K03781 - catalase [EC:1.11.1.6], katE, CAT, catB, srpA; Glyoxylate and dicarboxylate metabolism
K00246 - fumarate reductase subunit C, frdC; Citrate cycle (TCA cycle)
K01092 - myo-inositol-1(or 4)-monophosphatase [EC:3.1.3.25], E3.1.3.25, IMPA, suhB; Inositol phosphate metabolism
K00127 - formate dehydrogenase subunit gamma, fdoI, fdsG; Glyoxylate and dicarboxylate metabolism
K01512 - 
...
K01678 - fumarate hydratase subunit beta [EC:4.2.1.2] E4.2.1.2AB, fumB; Citrate cycle (TCA cycle)
K03737 - pyruvate-ferredoxin/flavodoxin oxidoreductase [EC:1.2.7.1 1.2.7.-], por, nifJ; Glycolysis / Gluconeogenesis
K01220 - 6-phospho-beta-galactosidase [EC:3.2.1.85], E3.2.1.85, lacG; Galactose metabolism
K00030 - isocitrate dehydrogenase (NAD+) [EC:1.1.1.41], IDH3; Citrate cycle (TCA cycle)
K02787 - ???
K02788 - lactose PTS system EIICB component [EC:2.7.1.207], lacE; Galactose metabolism
K00209 - enoyl-[acyl-carrier protein] reductase / trans-2-enoyl-CoA reductase (NAD+) [EC:1.3.1.9 1.3.1.44] fabV, ter; Butanoate metabolism
...
K02821 - ascorbate PTS system EIIA or EIIAB component [EC:2.7.1.194], ulaC, sgaA; Ascorbate and aldarate metabolism
K01433 - formyltetrahydrofolate deformylase [EC:3.5.1.10], purU; Glyoxylate and dicarboxylate metabolism
K00854 - xylulokinase [EC:2.7.1.17], xylB, XYLB; Pentose and glucuronate interconversions
K03079 - L-ribulose-5-phosphate 3-epimerase [EC:5.1.3.22], ulaE, sgaU, sgbU; Pentose and glucuronate interconversions
K00694 - cellulose synthase (UDP-forming) [EC:2.4.1.12], bcsA; Starch and sucrose metabolism
K03078 - 3-dehydro-L-gulonate-6-phosphate decarboxylase [EC:4.1.1.85], ulaD, sgaH, sgbH; Pentose and glucuronate interconversions
K02438 - glycogen debranching enzyme [EC:3.2.1.196], glgX; Starch and sucrose metabolism
K03335 - inosose dehydratase [EC:4.2.1.44], iolE; Inositol phosphate metabolism
K02782 - glucitol/sorbitol PTS system EIIB component [EC:2.7.1.198], srlE; Fructose and mannose metabolism

_PC2+:_
K00009 - mannitol-1-phosphate 5-dehydrogenase [EC:1.1.1.17], mtlD; Fructose and mannose metabolism
K01006 - pyruvate, orthophosphate dikinase [EC:2.7.9.1], ppdK; Glycolysis / Gluconeogenesis
K17489 - methylmalonyl-CoA carboxyltransferase 12S subunit [EC:2.1.3.1], E2.1.3.1-12S; Propanoate metabolism
K00008 - L-iditol 2-dehydrogenase [EC:1.1.1.14], SORD, gutB; Pentose and glucuronate interconversions
K01218 - mannan endo-1,4-beta-mannosidase [EC:3.2.1.78], gmuG; Fructose and mannose metabolism
K01580 - glutamate decarboxylase [EC:4.1.1.15], E4.1.1.15, gadB, gadA, GAD; Butanoate metabolism
K03737 - pyruvate-ferredoxin/flavodoxin oxidoreductase [EC:1.2.7.1 1.2.7.-], por, nifJ; Glycolysis / Gluconeogenesis
K03336 - 3D-(3,5/4)-trihydroxycyclohexane-1,2-dione acylhydrolase (decyclizing) [EC:3.7.1.22], iolD; Inositol phosphate metabolism
K00692 - levansucrase [EC:2.4.1.10], sacB; Starch and sucrose metabolism
K03416 - methylmalonyl-CoA carboxyltransferase 5S subunit [EC:2.1.3.1], E2.1.3.1-5S, pcyB; Propanoate metabolism

_PC2-:_
K07248 - lactaldehyde dehydrogenase / glycolaldehyde dehydrogenase [EC:1.2.1.22 1.2.1.21], aldA; Pyruvate metabolism
K01092 - myo-inositol-1(or 4)-monophosphatase [EC:3.1.3.25], E3.1.3.25, IMPA, suhB; Inositol phosphate metabolism
K18369 - alcohol dehydrogenase [EC:1.1.1.-], adh2; Propanoate metabolism
K01595 - phosphoenolpyruvate carboxylase [EC:4.1.1.31], ppc; Pyruvate metabolism
K03777 - D-lactate dehydrogenase (quinone) [EC:1.1.5.12], dld; Pyruvate metabolism
K00029 - malate dehydrogenase (oxaloacetate-decarboxylating)(NADP+) [EC:1.1.1.40], E1.1.1.40, maeB; Pyruvate metabolism
K01690 - phosphogluconate dehydratase [EC:4.2.1.12], edd; Pentose phosphate pathway
K00121 - S-(hydroxymethyl)glutathione dehydrogenase / alcohol dehydrogenase [EC:1.1.1.284 1.1.1.1], frmA, ADH5, adhC; Glycolysis / Gluconeogenesis
K01069 - hydroxyacylglutathione hydrolase [EC:3.1.2.6], gloB, gloc, HAGH; Pyruvate metabolism
K09788 - 2-methylaconitate isomerase [EC:5.3.3.-], prpF; Propanoate metabolism
