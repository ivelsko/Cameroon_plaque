...
mr1_species <- fread("./05-results.backup/minres_oblimin_cmc.fa_species.tsv") %>% 
  filter(Residual == "MR1") %>%
  select(Species) %>% 
  pull()

mr12_species <- fread("./05-results.backup/minres_oblimin_cmc.fa_species.tsv") %>% 
...
decayed_pca <- plot_pca_titles(malt_species.decontam_filtered, "PC1", "PC2", "Decayed", "Tooth_site", "Decayed", 3, "Decayed teeth, PC1-PC2")
decayed_pca
plot_pca_titles(malt_species.decontam_filtered, "PC3", "PC2", "Decayed", "Tooth_site", "Decayed", 3, "Decayed teeth, PC3-PC2")
# ggsave("./06-publication/supplemental_figures/SXX45/panel_parts/SXX45_decayed_pca_plot.pdf", plot = decayed_pca, device = "pdf",
#        scale = 1, width = 7, height = 5, units = c("in"), dpi = 300)

missing_pca <- plot_pca_titles(malt_species.decontam_filtered, "PC1", "PC2", "Missing", "Tooth_site", "Missing", 3, "Missing teeth, PC1-PC2")
missing_pca
plot_pca_titles(malt_species.decontam_filtered, "PC3", "PC2", "Missing", "Tooth_site", "Missing", 3, "Missing teeth, PC3-PC2")
# ggsave("./06-publication/supplemental_figures/SXX45/panel_parts/SXX45_missing_pca_plot.pdf", plot = missing_pca, device = "pdf",
#        scale = 1, width = 7, height = 5, units = c("in"), dpi = 300)

root_pca <- plot_pca_titles(malt_species.decontam_filtered, "PC1", "PC2", "Root_visible", "Tooth_site", "Root_visible", 3, "Root visible teeth, PC1-PC2")
root_pca
plot_pca_titles(malt_species.decontam_filtered, "PC3", "PC2", "Root_visible", "Tooth_site", "Root_visible", 3, "Root visible teeth, PC3-PC2")
# ggsave("./06-publication/supplemental_figures/SXX45/panel_parts/SXX45_root_pca_plot.pdf", plot = root_pca, device = "pdf",
#        scale = 1, width = 7, height = 5, units = c("in"), dpi = 300)

altered_pca <- plot_pca_titles(malt_species.decontam_filtered, "PC1", "PC2", "Altered", "Tooth_site", "Altered", 3, "Altered teeth, PC1-PC2")
altered_pca
plot_pca_titles(malt_species.decontam_filtered, "PC3", "PC2", "Altered", "Tooth_site", "Altered", 3, "Altered teeth, PC3-PC2")
# ggsave("./06-publication/supplemental_figures/SXX45/panel_parts/SXX45_altered_pca_plot.pdf", plot = altered_pca, device = "pdf",
#        scale = 1, width = 7, height = 5, units = c("in"), dpi = 300)
...
plot <- metadata %>%
  ...
  mutate(Decayed = ifelse(is.na(Decayed), "No", "Yes"),
         Counts = 1) %>%
  group_by(Ethnic_Group, Market_integration, Decayed) %>%
  count() %>%
  ggplot(., aes(x = Market_integration, y = n, fill = Decayed)) +
         geom_bar(stat = "identity", position = "dodge", color = "black") +
         scale_fill_manual(values = Decayed_colors) +
         theme_minimal(base_size = 14) +
         theme(axis.text.x = element_text(hjust = 0.5)) +
...
plot <- metadata %>%
  ...
  mutate(Missing = ifelse(is.na(Missing), "No", "Yes"),
         Counts = 1) %>%
  group_by(Ethnic_Group, Market_integration, Missing) %>%
  count() %>%
  ggplot(., aes(x = Market_integration, y = n, fill = Missing)) +
         geom_bar(stat = "identity", position = "dodge", color = "black") +
         scale_fill_manual(values = Missing_colors) +
         theme_minimal(base_size = 14) +
         theme(axis.text.x = element_text(hjust = 0.5)) +
...
# Root visible teeth
 plot <- metadata %>%
  ...
  mutate(Root_visible = ifelse(is.na(Root_visible), "No", "Yes"),
         Counts = 1) %>%
  group_by(Ethnic_Group, Market_integration, Root_visible) %>%
  count() %>%
  ggplot(., aes(x = Market_integration, y = n, fill = Root_visible)) +
         geom_bar(stat = "identity", position = "dodge", color = "black") +
         scale_fill_manual(values = Root_visible_colors) +
         theme_minimal(base_size = 14) +
         theme(axis.text.x = element_text(hjust = 0.5)) +
...
malt_species.decontam.alpha <- malt_species.decontam.alpha %>%
...
plot1 <- metadata %>%
  ...
  filter(str_detect(Individual_ID, "CMC")) %>%
    ggplot(., aes(x = Decayed_teeth, fill = Ethnic_Group)) +
         ...
         facet_wrap(~Ethnic_Group)
plot1
# ggsave("./06-publication/supplemental_figures/SXX44/panel_parts/SXX44_decayed_counts.pdf", plot = plot1, device = "pdf",
#        scale = 1, width = 7, height = 5, units = c("in"), dpi = 300)
...
plot1 <- metadata %>%
  ...
  distinct() %>%
  filter(str_detect(Individual_ID, "CMC")) %>%
    ggplot(., aes(x = Missing_teeth, fill = Ethnic_Group)) +
         geom_bar(color = "black") +
         scale_fill_manual(values = Ethnic_Group_colors) +
         theme_minimal(base_size = 14) +
         theme(axis.text.x = element_text(hjust = 0.5),
         ...
         facet_wrap(~Ethnic_Group)
plot1
# ggsave("./06-publication/supplemental_figures/SXX44/panel_parts/SXX44_missing_counts.pdf", plot = plot1, device = "pdf",
#        scale = 1, width = 7, height = 5, units = c("in"), dpi = 300)
...
plot1 <- metadata %>%
  ...
  distinct() %>%
  filter(str_detect(Individual_ID, "CMC")) %>%
    ggplot(., aes(x = Village, fill = `Decayed teeth`)) +
         geom_bar(color = "black") +
         ...
         xlab("Village")
plot1
# ggsave("./06-publication/supplemental_figures/SXX44/panel_parts/SXX44_decayed_village.pdf", plot = plot1, device = "pdf",
#        scale = 1, width = 7, height = 5, units = c("in"), dpi = 300)
...
plot1 <- metadata %>%
  ...
  filter(str_detect(Individual_ID, "CMC")) %>%
    ggplot(., aes(x = Village, fill = `Decayed teeth`)) +
         ...
         xlab("Village")
plot1
# ggsave("./06-publication/supplemental_figures/SXX44/panel_parts/SXX44_decayed_village_pct.pdf", plot = plot1, device = "pdf",
#        scale = 1, width = 7, height = 5, units = c("in"), dpi = 300)
...
plot1 <- metadata %>%
  ...
  summarize(Decayed_teeth = sum(No_decayed),
            Missing_teeth = sum(No_missing)) %>%
  mutate(`Missing teeth` = ifelse(Missing_teeth > 0, "Yes", "No")) %>%
  full_join(., metadata %>%
              select(Individual_ID, Ethnic_Group, Village, Market_integration, Age_group, Sex)) %>%
  distinct() %>%
  filter(str_detect(Individual_ID, "CMC")) %>%
    ggplot(., aes(x = Village, fill = `Missing teeth`)) +
         geom_bar(color = "black") +
         scale_fill_manual(values = c("#065FA3","#FF3200")) +
         theme_minimal(base_size = 14) +
         theme(axis.text.x = element_text(hjust = 0.5),
         ...
         xlab("Village")
plot1
# ggsave("./06-publication/supplemental_figures/SXX44/panel_parts/SXX44_missing_village.pdf", plot = plot1, device = "pdf",
#        scale = 1, width = 7, height = 5, units = c("in"), dpi = 300)
...
plot1 <- metadata %>%
  ...
  filter(str_detect(Individual_ID, "CMC")) %>%
    ggplot(., aes(x = Village, fill = `Missing teeth`)) +
         ...
         xlab("Village")
plot1
# ggsave("./06-publication/supplemental_figures/SXX44/panel_parts/SXX44_missing_village_pct.pdf", plot = plot1, device = "pdf",
#        scale = 1, width = 7, height = 5, units = c("in"), dpi = 300)
...
metadata %>%
  ...
  mutate(Ethnic_Group = as.character(Ethnic_Group)) %>%
  pairwise_wilcox_test(Missing_teeth ~ Ethnic_Group, p.adjust.method = "fdr")
...
metadata %>%
  ...
  # can't have factors for testing
  mutate(Ethnic_Group = as.character(Ethnic_Group)) %>%
  pairwise_wilcox_test(Decayed_teeth ~ Ethnic_Group, p.adjust.method = "fdr")
