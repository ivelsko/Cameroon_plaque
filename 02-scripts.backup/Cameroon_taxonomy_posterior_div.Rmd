...
malt_species.decontam_filtered_noblanks <- malt_species.decontam %>%
  ...
  select(-one_of(anterior)) %>% # leave only the posterior samples
...
# perform and plot a PCA to see how the data cluster
# by Village
sp_post_village <- plot_pca_titles(malt_species.decontam_filtered_noyanomami, "PC1", "PC2", "Village", "Ethnic_Group", "Ethnic_Group", 3)
sp_post_village
plot_pca_titles(malt_species.decontam_filtered_noyanomami, "PC3", "PC2", "Village", "Ethnic_Group", "Ethnic_Group", 3)

ggsave("./06-publication/supplemental_figures/SXX17/panel_parts/Figure_SXX17_sp_posterior_village_pc12.pdf", plot = sp_post_village, device = "pdf",
...
metadata_noblanks <- metadata_noblanks %>%
  ...
  filter(!str_detect(SampleID, "SRR164|ERR|SRS60|EXB|LIB|10M")) %>%
  filter(!SampleID %in% anterior) %>% # this is the group to remove
  as_tibble() %>%
...
# adonis(malt_species.decontam_filtered_noblanks_noyanomami.euc ~ Industry + Ethnic_Group + Age_group + Market_integration + Village, metadata_noblanks, perm=999)

perm <- how(nperm = 999)
setBlocks(perm) <- with(malt_species.decontam_filtered_noblanks_noyanomami.euc.vecmet, Tooth_site)
adonis2(malt_species.decontam_filtered_noblanks_noyanomami.euc ~  Ethnic_Group, malt_species.decontam_filtered_noblanks_noyanomami.euc.vecmet, permutations = perm, by = "margin")
...
# When only including posterior samples, none are significantly different
...
# perform and plot a PCA to see how the data cluster
# by Village
sp_post_village_cmc <- plot_pca_titles(malt_species.decontam_filtered_cmc, "PC1", "PC2", "Village", "Ethnic_Group", "Ethnic_Group", 3)
sp_post_village_cmc
plot_pca_titles(malt_species.decontam_filtered_cmc, "PC3", "PC2", "Village", "Ethnic_Group", "Ethnic_Group", 3)

ggsave("./06-publication/supplemental_figures/SXX17/panel_parts/Figure_SXX17_sp_posterior_village_cmc_pc12.pdf", plot = sp_post_village_cmc, device = "pdf",
       scale = 1, width = 7, height = 5, units = c("in"), dpi = 300)
...
adonis2(malt_species.decontam_filtered_cmc.euc ~  Ethnic_Group, malt_species.decontam_filtered_cmc.euc.vecmet, permutations = perm, by = "margin")

adonis2(malt_species.decontam_filtered_cmc.euc ~ ExtractionBatch + Village + Sex + Ethnic_Group + Age_group +  LibraryBatch + Market_integration, metadata_cmc, permutations = 999, by = "margin")
