...
songbird_noblanks_species_env <- fread("./04-analysis/songbird/species_noblanks/form1/differentials.tsv")
songbird_cmc_species_eg <- fread("./04-analysis/songbird/species_cmc/form1/differentials.tsv")
songbird_cmc_species_me <- fread("./04-analysis/songbird/species_cmc/form2/differentials.tsv")
songbird_cmc_species_ts <- fread("./04-analysis/songbird/species_cmc/form3/differentials.tsv")
songbird_cmc_species_sex <- fread("./04-analysis/songbird/species_cmc/form4/differentials.tsv")
...
ancomBC_cmc_sp_Fa_FC <- fread("./05-results.backup/ancomBC_species_noblanks_FA_FC.tsv")
ancomBC_cmc_sp_Fa_LR <- fread("./05-results.backup/ancomBC_species_noblanks_FA_LR.tsv")
...
songbird_noblanks_species_env %>%
  ...
  mutate(featureid = as_factor(featureid)) %>%
...
songbird_noblanks_species_env %>%
  ...
  arrange(`C(Env, Treatment('HunterGatherer'))[T.HMP]`) %>%
  mutate(featureid = as_factor(featureid)) %>%
  ggplot(., aes(x = featureid, y = `C(Env, Treatment('HunterGatherer'))[T.HMP]`)) +
    geom_col() +
    theme_minimal() +
...
songbird_noblanks_species_env %>%
  ...
  mutate(featureid = as_factor(featureid)) %>%
  ggplot(., aes(x = featureid, y = `C(Env, Treatment('HunterGatherer'))[T.JAE]`)) +
    geom_col() +
    theme_minimal() +
    theme(axis.text.x = element_text(angle = 90, hjust = 0.5)) +
    ylab("Log fold change") +
...
songbird_cmc_species_eg %>%
  ...
  mutate(featureid = as_factor(featureid)) %>%
  ggplot(., aes(x = featureid, y = `C(Ethnic_Group, Treatment('Nzime'))[T.Baka]`)) +
    geom_col() +
    theme_minimal() +
    theme(axis.text.x = element_text(angle = 90, hjust = 0.5)) +
    ylab("Log fold change") +
...
songbird_cmc_species_me %>%
  ...
  mutate(featureid = as_factor(featureid)) %>%
  ggplot(., aes(x = featureid, y = `C(Market_economy, Treatment('Farming'))[T.Logging road]`)) +
    geom_col() +
    theme_minimal() +
    theme(axis.text.x = element_text(angle = 90, hjust = 0.5)) +
    ylab("Log fold change") +
...
songbird_cmc_species_ts %>%
  filter(`C(Tooth_site, Treatment('Posterior'))[T.Anterior]` >= 1 | `C(Tooth_site, Treatment('Posterior'))[T.Anterior]` <= -1) %>%
  arrange(`C(Tooth_site, Treatment('Posterior'))[T.Anterior]`) %>%
  mutate(featureid = as_factor(featureid)) %>%
  ggplot(., aes(x = featureid, y = `C(Tooth_site, Treatment('Posterior'))[T.Anterior]`)) +
    geom_col() +
    theme_minimal() +
    theme(axis.text.x = element_text(angle = 90, hjust = 0.5)) +
    ylab("Log fold change") +
...
songbird_cmc_species_sex %>%
  ...
  mutate(featureid = as_factor(featureid)) %>%
  ggplot(., aes(x = featureid, y = `C(Sex, Treatment('Male'))[T.Female]`)) +
    geom_col() +
    theme_minimal() +
    theme(axis.text.x = element_text(angle = 90, hjust = 0.5)) +
    ylab("Log fold change") +
