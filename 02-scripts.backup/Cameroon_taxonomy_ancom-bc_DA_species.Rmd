...
fwrite(ancom_bc_noblanks_HMP_HG, "./05-results.backup/ancomBC_species_noblanks_HMP_HG.tsv", sep = "\t", quote = F) # equivalent to ma_us_phylum_age2.csv in the example below
...
fwrite(ancom_bc_noblanks_JAE_HG, "./05-results.backup/ancomBC_species_noblanks_JAE_HG.tsv", sep = "\t", quote = F)
...
ancom_HMP_CMC_plot <- res.ANCOM_BC_env_HMP_HG %>%
  filter(`log fold change (HunterGatherer - HMP)` >= 4.0 | `log fold change (HunterGatherer - HMP)` <= -4.0) %>%
  arrange(`log fold change (HunterGatherer - HMP)`) %>%
  mutate(Species = as_factor(Species)) %>%
  ggplot(., aes(x = Species, y = `log fold change (HunterGatherer - HMP)`, ymin = ci.lo.adj, ymax = ci.up.adj, fill = diff.abn)) +
    geom_col() +
    ...
    geom_point(data = res.ANCOM_BC_env_HMP_HG %>% filter(`structural.zero (HMP)` == 1) %>%
                      filter(`log fold change (HunterGatherer - HMP)` >= 4.0 | `log fold change (HunterGatherer - HMP)` <= -4.0) %>%
                      arrange(`log fold change (HunterGatherer - HMP)`) %>%
                      mutate(Species = as_factor(Species)), 
               aes(x=Species, y=`log fold change (HunterGatherer - HMP)`),
...
ancom_JAE_CMC_plot <- res.ANCOM_BC_env_JAE_HG %>%
  filter(`log fold change (HunterGatherer - JAE)` >= 4.0 | `log fold change (HunterGatherer - JAE)` <= -4.0) %>%
  arrange(`log fold change (HunterGatherer - JAE)`) %>%
  mutate(Species = as_factor(Species)) %>%
  ggplot(., aes(x = Species, y = `log fold change (HunterGatherer - JAE)`, ymin = ci.lo.adj, ymax = ci.up.adj, fill = diff.abn)) +
    geom_col() +
    geom_errorbar(width=0.2, size=0.25, position=position_dodge(width = 0.4)) +
    geom_abline(slope = 0, intercept = 0, linetype="dashed", alpha=0.5) +
    # coord_flip() +
    ...
    geom_point(data = res.ANCOM_BC_env_JAE_HG %>% filter(`structural.zero (JAE)` == 1) %>%
                      ...
                      arrange(`log fold change (HunterGatherer - JAE)`) %>%
                      mutate(Species = as_factor(Species)), 
               aes(x=Species, y=`log fold change (HunterGatherer - JAE)`),
                   position=position_dodge(width = 0.4), shape=18) +
...
ancom_JAE_CMC_plot
 ...
 # ggsave("~/Desktop/ancom_HMP_CMC_plot.png", plot = ancom_HMP_CMC_plot, device = "png",
 #        scale = 1, width = 8, height = 4, units = c("in"), dpi = 300)
 # 
 # ggsave("~/Desktop/ancom_JAE_CMC_plot.png", plot = ancom_JAE_CMC_plot, device = "png",
 #        scale = 1, width = 8, height = 4, units = c("in"), dpi = 300)
...
ma_us_18_40=read_csv("./05-results.backup/ma_us_phylum_age18_40.csv") # JAE-HG
...
res.ANCOM_BC_Fa_FC_star %>%
  filter(`log fold change (ForrestCamp - Farming)` >= 1.5 | `log fold change (ForrestCamp - Farming)` <= -1.5) %>%
...
res.ANCOM_BC_ts %>%
  filter(`log fold change (Anterior - Posterior)` >= 2.5 | `log fold change (Anterior - Posterior)` <= -2.5) %>%
