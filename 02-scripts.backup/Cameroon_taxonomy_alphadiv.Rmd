...
# make a heat map for just the CMC samples (get the sample order to be by village!!!)
...
malt_species.decontam_top50_each <- malt_species.decontam_top50_cmcV %>%
...
all3 <- malt_species.decontam_top50_cmcV %>%
  inner_join(., malt_species.decontam_top50_hmpV) %>%
  inner_join(., malt_species.decontam_top50_jaeV) 

jaehmp <- malt_species.decontam_top50_hmpV %>%
  inner_join(., malt_species.decontam_top50_jaeV)

cmcjae <- malt_species.decontam_top50_cmcV %>%
  inner_join(., malt_species.decontam_top50_jaeV)

cmchmp <- malt_species.decontam_top50_cmcV %>%
  inner_join(., malt_species.decontam_top50_hmpV)

cmc_unique <- malt_species.decontam_top50_cmcV %>%
  anti_join(., malt_species.decontam_top50_hmpV) %>%
  anti_join(., malt_species.decontam_top50_jaeV)
...
heatmap_base <- ggplot(malt_species.decontam_top50_all_clr_long, aes(SampleID, Genus, fill = CLR)) +
  geom_tile() +
  scale_fill_viridis_c(option = "C") +
  theme_minimal(base_size = 7) +
  theme(axis.text.x = element_text(angle = 90),
