...
```{r setup, eval=F}
knitr::opts_knit$set(root.dir = normalizePath("/Users/oval_portrait/projects1/microbiome_calculus/cameroon_hunter_gatherer_calculus"))
```
...
# malt_species <- fread("/Users/oval_portrait/projects1/microbiome_calculus/cameroon_hunter_gatherer_calculus/04-analysis/malt/tables.backup/CMC_MALT_nt_species_bacteria_archaea.txt")
# malt_genus <- fread("/Users/oval_portrait/projects1/microbiome_calculus/cameroon_hunter_gatherer_calculus/04-analysis/malt/tables.backup/CMC_MALT_nt_genus_bacteria_archaea.txt")
# malt_species <- fread("/Users/oval_portrait/Dropbox/MPI-SHH/CMC/CMC_MALT_nt_species_bacteria_archaea.txt")
# malt_genus <- fread("/Users/oval_portrait/Dropbox/MPI-SHH/CMC/CMC_MALT_nt_genus_bacteria_archaea.txt")


# malt_species <- fread("/Users/oval_portrait/projects1/microbiome_calculus/cameroon_hunter_gatherer_calculus/04-analysis/malt/tables.backup/CMC_MALT_RSC_species_bacteria_archaea.txt")
# malt_genus <- fread("/Users/oval_portrait/projects1/microbiome_calculus/cameroon_hunter_gatherer_calculus/04-analysis/malt/tables.backup/CMC_MALT_RSC_genus_bacteria_archaea.txt")


# malt_species <- fread("/Users/oval_portrait/projects1/microbiome_calculus/cameroon_hunter_gatherer_calculus/04-analysis/malt/tables.backup/CMC_someHMP_MALT_RSC_species_bacteria_archaea.txt")
# malt_genus <- fread("/Users/oval_portrait/projects1/microbiome_calculus/cameroon_hunter_gatherer_calculus/04-analysis/malt/tables.backup/CMC_someHMP_MALT_RSC_genus_bacteria_archaea.txt")

malt_species <- fread("/Users/oval_portrait/projects1/microbiome_calculus/cameroon_hunter_gatherer_calculus/04-analysis/malt/tables.backup/CMC_HMP_MALT_RSC_species_bacteria_archaea.txt")
malt_genus <- fread("/Users/oval_portrait/projects1/microbiome_calculus/cameroon_hunter_gatherer_calculus/04-analysis/malt/tables.backup/CMC_HMP_MALT_RSC_genus_bacteria_archaea.txt")
...
metadata <- fread("/Users/oval_portrait/projects1/microbiome_calculus/cameroon_hunter_gatherer_calculus/00-documentation.backup/01-cameroon_hunter_gatherer_metadata.tsv")
# metadata <- fread("/Users/oval_portrait/Dropbox/MPI-SHH/CMC/01-cameroon_hunter_gatherer_metadata.tsv")
...
market_economy_colors <- c(logging_road = "#FF3200", forrest_camp = "#172869", other = "#82CDAA", HMP = "#969696", HMP10M = "#d9d9d9",
                           ExtractionGauze = "#252525", ExtractionBlank = "#737373", LibraryBlank = "#bdbdbd")
market_economy_shapes <- c(logging_road = 19, forrest_camp = 17, other = 18, HMP = 7, HMP10M = 9,
                           ExtractionGauze = 8, ExtractionBlank = 4, LibraryBlank = 3)
...
```{r outliers}
outliers <- c("CMC002.B0201","CMC032.B0101","CMC037.B0201","CMC047.A0201","EXB059.A1901","EXB059.A1301","EXB059.A1101")

metadata %>% select(one_of("SampleID","Individual_Seq_Depth_NonHuman","MALT_aligned_reads","Percentage_Hits_with_Taxonomy")) %>% filter(str_detect(SampleID, "CMC002.A0201|CMC037.B0201|CMC047.A0201|CMC014.A0101|CMC005.B0101|CMC031.A0201"))
...
```{r contaminants}
contaminants_species <- cont_comb_species_list %>% rownames_to_column("Species") %>% select(Species)
malt_species.decontam <- malt_species %>%
  # gather("SampleID","Counts", 2:ncol(.)) %>%
  # spread(Species, Counts) %>%
  anti_join(., contaminants_species)

# write.table(malt_species.decontam, file = "/Users/oval_portrait/Dropbox (Personal)/MPI-SHH/CMC/malt_species.decontam.tsv", sep="\t", quote=F, row.names = F)
# write.table(malt_species, "/Users/oval_portrait/Dropbox (Personal)/MPI-SHH/CMC/malt_species.tsv", sep="\t", quote=F, row.names = F)

contaminants_genus <- cont_comb_genus_list %>% rownames_to_column("Species") %>% select(Species)
...
# prepare to run a PCA
# check the number of components to retain by tuning the PCA
tune.nt_species_pca <- tune.pca(malt_species_filtered, logratio = 'CLR')
tune.nt_species_pca

# perform a PCA to see how the data cluster
nt_species.pca <- mixOmics::pca(malt_species_filtered, ncomp = 3, logratio = 'CLR')
plot(nt_species.pca)
...
pca_nt_species_pc1pc2 <- ggplot(nt_species.pca.varmet, aes(PC1, PC2, colour = Individual_Seq_Depth_NonHuman, shape = Env)) +
  geom_point(size = 4) +
  scale_shape_manual(values = Env_shapes) +
  xlab(paste("PC1 - ", round(nt_species.pca$explained_variance[1], 2), sep = "")) +
  ylab(paste("PC2 - ", round(nt_species.pca$explained_variance[2], 2), sep = "")) +
...
# ggsave("/Users/oval_portrait/Dropbox (Personal)/MPI-SHH/CMC/pca_nt_species_pc1pc2.pdf", plot = pca_nt_species_pc1pc2, device = "pdf",
...
# prepare to run a PCA
# check the number of components to retain by tuning the PCA
tune.nt_species.decontam_pca <- tune.pca(malt_species.decontam_filtered, logratio = 'CLR')
tune.nt_species.decontam_pca

# perform a PCA to see how the data cluster
nt_species.decontam.pca <- mixOmics::pca(malt_species.decontam_filtered, ncomp = 3, logratio = 'CLR')
plot(nt_species.decontam.pca)

# Select out the PC variates into a new table and add the metadata for plotting
nt_species.decontam.pca.variates <- as.data.frame(nt_species.decontam.pca$variates$X)
nt_species.decontam.pca.variates <- nt_species.decontam.pca.variates %>%
  rownames_to_column("SampleID")
  
nt_species.decontam.pca.varmet <- inner_join(nt_species.decontam.pca.variates, metadata, by = "SampleID")

pca_nt_species.decontam_pc1pc2 <- ggplot(nt_species.decontam.pca.varmet, aes(PC1, PC2, colour = Env, shape = Env)) +
  geom_point(size = 4) +
  scale_colour_manual(values = Env_colors) +
  scale_shape_manual(values = Env_shapes) +
  xlab(paste("PC1 - ", round(nt_species.decontam.pca$explained_variance[1], 2), sep = "")) +
  ylab(paste("PC2 - ", round(nt_species.decontam.pca$explained_variance[2], 2), sep = "")) +
  theme_minimal(base_size = 14) 

pca_nt_species.decontam_pc1pc2

pca_nt_species.decontam_pc1pc2 <- ggplot(nt_species.decontam.pca.varmet, aes(PC1, PC2, colour = Individual_Seq_Depth_NonHuman, shape = Env)) +
  geom_point(size = 4) +
  scale_shape_manual(values = Env_shapes) +
  xlab(paste("PC1 - ", round(nt_species.decontam.pca$explained_variance[1], 2), sep = "")) +
  ylab(paste("PC2 - ", round(nt_species.decontam.pca$explained_variance[2], 2), sep = "")) +
...
# ggsave("/Users/oval_portrait/Dropbox (Personal)/MPI-SHH/CMC/pca_nt_genus_pc1pc2.pdf", plot = pca_nt_genus_pc1pc2, device = "pdf",
#        scale = 1, width = 7, height = 5, units = c("in"),
#        dpi = 300)
...
malt_species.decontam_filtered.os <- presabs_table %>%
...
# make the poplabel a factor for plotting
...
malt_genus.decontam_filtered.os <- presabs_table %>%
