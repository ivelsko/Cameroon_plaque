...
Market_economy_colors <- c(logging_road = "#FF3200", forrest_camp = "#172869", farming = "#82CDAA", Yanomami = "#C70E7B", Industrial = "#969696",
                           ExtractionGauze = "#252525", ExtractionBlank = "#737373", LibraryBlank = "#bdbdbd")
Market_economy_shapes <- c(logging_road = 19, forrest_camp = 17, farming = 18, Yanomami = 14, Industrial = 15,
                           ExtractionGauze = 8, ExtractionBlank = 4, LibraryBlank = 3)
...
are aerobic. *Really need to see which HMP samples are supragingival and which*
*are subgingival.* All Yanomami buccal swab samples have negative PC2 values, and
it makes sense that *Neisseria* and other aerobic taxa characterize this
direction, as they are more abundant on buccal epithelia. In PC2, the positive
loading taxa are anaerobic, later colonizers, which corresponds with the
calculus samples that plot in positice PC2 values. The PC2 negative loading taxa
are a mix of aerobes, facultative anaerobes, and anaerobes, characteristic of a
younger biofilm community such as found in plaque compared to calculus. In PC3,
the positive loading taxa are mostly *Neisseria*, with one *Strep* & one *Rothia*,
while the negative loading taxa are mostly *Rothia*, with 2 *Actinomyces.* Both of
these groups are mostly aerobic, but suggests that *Rothia* are more predominant
in plaque than *Neisseria*, while *Neisseria* are more predominant in buccal swabs.


```{r biplot_samples_species_no_yanomami}
...
malt_species_decontam_filtered_noyanomami_biplot_121 <- plot_pca_bi(malt_species.decontam_filtered_noblanks_noyanomami.pca, "PC1", "PC2", "Village", PC1)
...
malt_species_decontam_filtered_noyanomami_biplot_122 <- plot_pca_bi(malt_species.decontam_filtered_noblanks_noyanomami.pca, "PC1", "PC2", "Village", PC2)
...
malt_species_decontam_filtered_noyanomami_biplot_322 <- plot_pca_bi(malt_species.decontam_filtered_noblanks_noyanomami.pca, "PC3", "PC2", "Village", PC2)
malt_species_decontam_filtered_noyanomami_biplot_322

malt_species_decontam_filtered_noyanomami_biplot_323 <- plot_pca_bi(malt_species.decontam_filtered_noblanks_noyanomami.pca, "PC3", "PC2", "Village", PC3)
...
suggesting they are more mature or exposed to lower oxygen tension. *Does this*
*separation correspond to the separation of anterior/posterior tooth sites?* In
PC2 positive loadings there is a mix of aerobic and anaerobic taxa, with more
...
of plaque in positive values and calculus in negative values. In PC3, the
positive loading taxa are a mix of aerobic and anaerobic taxa, while the
negative loading taxa are also a mix, but *Neisseria* make up 6 of the 10.
...
metadata_noblanks <- metadata_noblanks %>%
  ...
  as_tibble() %>%
  mutate(Market_economy = str_replace(Market_economy, "HMP","Industrial"),
         Market_economy = str_replace(Market_economy, "logging_road","Logging road"),
         Market_economy = str_replace(Market_economy, "forrest_camp","Forrest camp"),
         Market_economy = str_replace(Market_economy, "farming","Farming"))%>%
  mutate(Ethnic_Group = fct_relevel(Ethnic_Group, "Baka","Nzime","HMP","JAE"),
         ...
         Sex = fct_relevel(Sex, "Male","Female","HMP"),
...
malt_species.decontam_filtered_noblanks_noyanomami.euc.vecmet %>%
  ggplot(., aes(Axis.1, Axis.2, colour = Market_economy, shape = Market_economy)) +
  geom_point(size = 3) +
  scale_colour_manual(values = Market_economy_colors) +
  scale_shape_manual(values = Market_economy_shapes) +
  stat_ellipse(aes(colour = Market_economy, group = Market_economy)) +
  theme_minimal(base_size = 7) +
  ggtitle("Tooth site")

# test for difference between different metadata categories 
# Sex can't be run b/c for JAE012 sex is unknown, and is NA
adonis(malt_species.decontam_filtered_noblanks_noyanomami.euc ~ Tooth_site + Ethnic_Group + Age_group + Market_economy + Village, malt_species.decontam_filtered_noblanks_noyanomami.euc.vecmet, perm=999)
...
```{r betadisper_noblanks_species, eval = F}
groups <- metadata_noblanks %>%
  select(Ethnic_Group) %>%
  pull()

## Calculate multivariate dispersions
mod <- betadisper(malt_species.decontam_filtered_noblanks_noyanomami.euc, groups, bias.adjust =TRUE)
mod
...
# test for difference between different metadata categories 
adonis(malt_species.decontam_filtered_cmc.euc ~ Tooth_site + Ethnic_Group + Market_economy + Sex + Village + Age_group, malt_species.decontam_filtered_cmc.euc.vecmet, perm=999)
...
metadata_cmc <- metadata_cmc %>%
  ...
  as_tibble() %>%
  mutate(Market_economy = str_replace(Market_economy, "logging_road","Logging road"),
         Market_economy = str_replace(Market_economy, "forrest_camp","Forrest camp"),
         Market_economy = str_replace(Market_economy, "farming","Farming")) %>%
...
Then run betadisper to see if the groups have different dispersion ("tight" vs. "loose" clustering of groups) - is one group more variable than the other(s)?
```{r betadisper_cmc_species, eval = F}
...
## Calculate multivariate dispersions
mod <- betadisper(malt_species.decontam_filtered_cmc.euc, groups, bias.adjust =TRUE)
mod
...
level than the species level. *Are those HMP samples supragingival plaque??*  
...
direction in the species analysis.


```{r biplot_samples_genus_no_yanomami}
...
malt_genus_decontam_filtered_noyanomami_biplot_121 <- plot_pca_bi(malt_genus.decontam_filtered_noblanks_noyanomami.pca, "PC1", "PC2", "Village", PC1)
...
malt_genus_decontam_filtered_noyanomami_biplot_122 <- plot_pca_bi(malt_genus.decontam_filtered_noblanks_noyanomami.pca, "PC1", "PC2", "Village", PC2)
...
malt_genus_decontam_filtered_noyanomami_biplot_322 <- plot_pca_bi(malt_genus.decontam_filtered_noblanks_noyanomami.pca, "PC3", "PC2", "Village", PC2)
malt_genus_decontam_filtered_noyanomami_biplot_322

malt_genus_decontam_filtered_noyanomami_biplot_323 <- plot_pca_bi(malt_genus.decontam_filtered_noblanks_noyanomami.pca, "PC3", "PC2", "Village", PC3)
...
plotting in negative PC1 values. In PC2, the positive loading taxa are mostly
later colonizing anaerobes, which corresponds with the JAE calculus samples
plotting in this part of the graph. The negative loading taxa are a mix of
aerobic and facultative taxa, which corresponds with the HMP plaque samples
plotting in this part of the graph. In PC3, the positive loading taxa are
unusual, rare, or poorly characterized in the oral biofilm, while the negative
loading taxa are mostly later colonizer, anaerobes. Several calculus samples
plot far in PC3 positive values, which suggests that there are some unusual,
poorly characterized taxa in calculus that are not seen in plaque.
...
# test for difference between different metadata categories 
# Sex can't be run b/c for JAE012 sex is unknown, and is NA
adonis(malt_genus.decontam_filtered_noblanks_noyanomami.euc ~ Tooth_site + Ethnic_Group + Age_group + Market_economy + Village, malt_genus.decontam_filtered_noblanks_noyanomami.euc.vecmet, perm=999)


```

Then run betadisper to see if the groups have different dispersion ("tight" vs. "loose" clustering of groups) - is one group more variable than the other(s)?
```{r betadisper_noblanks_genus, eval = F}
groups <- metadata_noblanks %>%
  select(Ethnic_Group) %>%
  pull()

## Calculate multivariate dispersions
mod <- betadisper(malt_genus.decontam_filtered_noblanks_noyanomami.euc, groups, bias.adjust =TRUE)
mod
