...
malt_species.decontam_filtered_cmc.euc.vecmet %>%
  ggplot(., aes(Axis.1, Axis.2, colour = Tooth_site, shape = Sex)) +
  geom_point(size = 3) +
...
the postive loading taxa are mostly later colonizer anaerobes, while the
negative loading taxa are mix of aerobes and facultative anaerobes. This
corresponds with the separation of calculus in positive PC2 values (more mature
community) and plaque on negative values (less mature community). There is
almost no overlap of species and genera in PC2 loadings. In PC3, the positive
loading taxa are mostly later colonizing anaerobes, while the negative loading
taxa are unusual, rare, or poorly characterized in the oral biofilm. There is no
overlap in the species and genera in PC3 loadings. The Yanomami samples again
separate from JAE and CMC along PC3, but overlap with HMP. However, they
separate from HMP along PC2. It is unexpected that none of the genera with PC3
positive loadings are the aerobic taxa that characterized this direction in the
species analysis.
...
```
When excluding Yanomami samples, the patterns of taxa characterizing sample group separation are still less clear at the genus level than they are at the species level. In PC1, the positive loading taxa are mostly later colonizing anaerobes, while the positive loading taxa are more aerobic and facultative anaerobic genera. The taxa characterizing this direction corresond with the HMP plaque plotting in negative PC1 values, which would have a less mature biofilm community. In PC2, the positive loading taxa are mostly later colonizing anaerobes, which corresponds with the JAE calculus samples plotting in this part of the graph. The negative loading taxa are a mix of aaerobic and facultative taxa, which corresponds with the HMP plaque samples plotting in this part of the graph. In PC3, the positive loading taxa are unusual, rare, or poorly characterized in the oral biofilm, while the negative loading taxa are also unusual, rare, or poorly characterized in the oral biofilm.
